...
import { savePrompt, getAllUIConnections } from './database-helpers.mjs';
...
export const lambdaHandler = async (event, context) => {
    ...
    let ui_ws_domain_name = 'ws3n0jhyr6.execute-api.us-east-1.amazonaws.com';
    ...
    try {
        ...
        if (body?.type === "prompt" || body?.type === "dtmf") {
            ...
            console.log("put new websocket conn and CR conn Item: " + JSON.stringify(putItem));
        ...
        if (uiConnection != null) {
            ...
            console.log("about to send these messages down the ui websocket: " + JSON.stringify(ui_messages));
...
async function sendMessages(array, ws_client, connId) {
    for (const item of array) {
        console.log("sending message: " + JSON.stringify(item) + "\nto ui websocket with connId: " + connId)
