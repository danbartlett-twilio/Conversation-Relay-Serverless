...
async function listAvailableApartments(ddbDocClient, tool) {
  ...
  const apartmentDetailsRaw = await ddbDocClient.send(
    new GetCommand({
      TableName: process.env.TABLE_NAME,
      Key: { pk: "apartments", sk: "database" },
    })
  );

  const apartmentDetails = JSON.parse(
    apartmentDetailsRaw.Item.apartmentDetails
  );
...
export async function ListAvailableApartmentsFunction(ddbDocClient, tool) {
  ...
  try {
    ...
    console.log(
      "Error failed to complete the function [ListAvailableApartmentsFunction] => ",
      error
    );
