...
async function scheduleTour(ddbDocClient, tool) {
  const { date, time, type, apartmentType } = JSON.parse(
    tool.function.arguments
  );

  const apartmentsRaw = await ddbDocClient.send(
    new QueryCommand({
  ...
  */
  ...
  const deleteAppointment = await ddbDocClient.send(
    new DeleteCommand({
      TableName: process.env.TABLE_NAME,
      Key: {
  ...
  let userPk = tool.userContext?.pk
    ? tool.userContext.pk
    : tool.call_details.from_phone;
...
export async function ScheduleTourFunction(ddbDocClient, tool) {
  ...
  try {
    ...
    console.log(
      "Error failed to complete the function [ScheduleTourFunction] => ",
      error
    );
