...
export const lambdaHandler = async (event, context) => {
  ...
  const useCase = await ddbDocClient.send(
    new GetCommand({
      TableName: process.env.TABLE_NAME,
      Key: { pk: useCaseTitle, sk: "configuration" },
    })
  );
  ...
  if (user.Item) {
    ...
    if (user.Item.email) {
      rString =
        rString +
        ` Their email address is ${user.Item.email} -- you may offer to send emails.`;
    } else {
      rString =
        rString +
        ` You do not have their email address so cannot offer to send emails.`;
    }
    prompt = prompt.replace("<<USER_CONTEXT>>", rString);
  } else {
    prompt = prompt.replace(
      "<<USER_CONTEXT>>",
      `First ask for the user's first and last name to use during the call.`
    );
  }
