...
async function sendEmail(ddbDocClient, tool) {
  ...
  const message = `<b>Hi ${name}, Twilio Dough Boys has accepted your order [${order_items_text.join(
    ", "
  ...
  console.log(
    `[SendOrderConfirmationMessage] email sent successfully: ${snsResult}`
  );
...
export async function SendRestaurantEmailFunction(ddbDocClient, tool) {
  console.info(
    "in SendRestaurantEmailFunction and tool\n" + JSON.stringify(tool, null, 2)
  );
