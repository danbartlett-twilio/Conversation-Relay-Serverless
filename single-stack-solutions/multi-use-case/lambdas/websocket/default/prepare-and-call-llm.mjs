...
export async function prepareAndCallLLM(prepareObj) {
  ...
  if (prepareObj.body?.type === "prompt") {
    // VOICE PROMPT EVENT

    // Format the prompt from the user to LLM standards.
    let newUserChatMessage = {
      role: "user",
      content: prepareObj.body.voicePrompt,
    };
  ...
  } else if (prepareObj.body?.type === "dtmf") {
    // DTMF EVENT
    ...
    if (!dtmfResponse) {
      ...
      throw new Error(
        "Could not find DTMF handler for current use case / digit."
      );
    } else {
      ...
      if (dtmfResponse.replyWithText) {
        newUserDTMFMessage.content =
          newUserDTMFMessage.content +
          ` Reply to the user with this text: "${dtmfResponse.replyText}"`;
      ...
      if (dtmfResponse.replyWithFunction) {
        ...
        tool_choice = {
          type: "function",
          function: { name: dtmfResponse.replyFunction },
        };
        newUserDTMFMessage.content =
          newUserDTMFMessage.content + " Call the tool and wait.";
      }
  ...
  } else if (prepareObj.toolCallCompletion) {
